...
Overrides ActiveSupport::Dependencies#<tt>new_constants_in</tt>.
=end
...
module ActiveSupport::Dependencies
  ...
  def inject_dependency(target, *requirements)
    ...
    _logger_debug "injection graph: #{injection_graph.inspect}" if ActiveSupport::Dependencies.log_activity
  end
  ...
  def new_constants_in_with_injection(*descs, &block) # chain
    ...
    if ActiveSupport::Dependencies.log_activity
    ...
    returning(new_constants_in_without_injection(*descs, &block)) do |found|
      _logger_debug "new constants: #{found.inspect}" if ActiveSupport::Dependencies.log_activity and found.any?
      found.each do |constant|
        injection_graph[constant].each do |requirement| 
          requirement.constantize
           _logger_debug "constantized #{requirement}" if ActiveSupport::Dependencies.log_activity
        end
      end    
    end
  end
